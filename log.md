...
* **2026-07-25 — Stratégie RAG Avancée (Inspirée de DocsGPT)** :
  - **Objectif** : fiabiliser les réponses générées par le LLM, éviter que des requêtes techniques (ex. scripts SQL/Python longs) ne soient tronquées, et supprimer les hallucinations.
  - **Chunking Parent-Enfant** : modification profonde de `parsing.chunk_elements`. Les éléments sont accumulés dans de larges blocs de ~1200 caractères ("Parents") pour préserver l'intégralité du contexte (ex: un long script). Ces blocs sont ensuite sous-découpés en "Enfants" de ~300 caractères (avec 50 car. d'overlap). L'embedding `bge-m3` se fait sur le bloc Enfant pour maximiser la pertinence vectorielle, tandis que le texte Parent complet est stocké dans la nouvelle colonne `parent_text`.
  - **Schéma DB** : ajout de la colonne `parent_text TEXT` dans la table `chunks` via le script de migration `migrate-db/run.py`.
  - **Recherche & Prompt XML** : la recherche hybride (`kb.hybrid_search_chunks`) renvoie désormais le contexte étendu (`parent_text`). L'API (`web/app.py`) et l'outil de test (`tests/manual_ollama.py`) formatent ce contexte avec des balises XML strictes (`<document><source>...</source><content>...</content></document>`).
  - **Prompt Anti-Hallucination** : adoption du prompt système strict de DocsGPT, forçant le modèle à répondre uniquement à partir du contexte fourni, à citer précisément les sources en fin de phrase, et à décliner si la réponse n'y figure pas.
  - **Résultat** : les tests montrent un respect absolu du contexte, le maintien intact des scripts SQL extraits, et une citation systématique de la source originelle. Base entièrement réinitialisée et indexation relancée.
